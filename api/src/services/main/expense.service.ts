...
export class ExpenseService {
    ...
    public async createExpense(req: Request): Promise<ExpenseInterface> {
        const dto = await DtoFactory.create<ExpensePostDto>(ExpensePostDto, req.body as Partial<ExpensePostDto>);
    ...
    public async editExpense(req: Request): Promise<ExpenseInterface> {
        const expense = await this.expenseHelper.findOneOrThrowException(req.params.id);
        const oldExpense = { ...expense.toObject() };
        const dto = await DtoFactory.create<ExpensePatchDto>(ExpensePatchDto, req.body as Partial<ExpensePatchDto>);
        expense.set(dto);
        await expense.save();
        ...
        this.logger.info(
            `User ${req.user.username} edited expense from  ${JSON.stringify(oldExpense)} to ${JSON.stringify(
                expense,
            )}`,
        );
        return expense;
    ...
    public async getExpense(req: Request): Promise<ExpenseInterface> {
        const expense = await this.expenseHelper.findOneOrThrowException(req.params.id);
        const ifParticipant =
            expense.participants.filter((el: RelatedUserDto) => el.username === req.user.username).length > 0;

        if (!ifParticipant) {
            throw new HttpExceptions.AccessDenied('Only participant has access to expense details');
        }
    ...
    public async getByMonth(month: string, filters: any): Promise<any> {
        const balance = await this.expenseHelper.getMonthlyBalance(month, filters);
