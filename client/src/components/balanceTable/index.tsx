...
export const BalanceTable = ({ balance, total }: BalanceTableType) => {
    const totalSum: number = balance.map((item) => item.totalAmount).reduce((prev, curr) => prev + curr, 0);
                    ...
                    ? balance.map((b, i) => {
                          return (
                              <tr className='bg-gray-800' key={i}>
                                  ...
                                  <td className='px-6 py-4'>{b.expensesCount}</td>
                                  <td className='px-6 py-4'>{b.totalAmount + ' zł'}</td>
                              </tr>
                          );
                      })
                    : null}
